    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-5 py-10 lg:grid-cols-[minmax(360px,0.82fr)_minmax(520px,1.18fr)] lg:px-8 lg:py-14">
        ...
        <section className="document-shell" aria-live="polite">
          <div className="flex items-center justify-between border-b border-border px-6 py-5"><div><p className="eyebrow">Output / 02</p><h2 className="mt-1 font-serif text-2xl font-semibold text-primary">Meeting minutes</h2></div><div className="flex gap-2">{output && <><button onClick={copy} className="secondary-button" title="Copy minutes"><Clipboard size={15} /> <span className="hidden sm:inline">Copy</span></button><button onClick={() => download(output)} className="secondary-button" title="Download minutes"><FileText size={15} /> <span className="hidden sm:inline">Export</span></button></>}</div></div>
          {output ? <div className="space-y-7 p-6 sm:p-8"><div><h3 className="font-serif text-3xl font-semibold leading-tight text-primary">{output.title}</h3><div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-xs text-muted-foreground"><span>{output.date}</span><span>{output.time}</span><span>{output.location}</span></div></div><DocumentSection label="Attendees" content={output.attendees} /><DocumentSection label="Discussion" items={output.discussions} /><DocumentSection label="Decisions" items={output.decisions} /><DocumentSection label="Action items" items={output.actions} numbered />{output.review.length > 0 && <div className="review-note"><RefreshCw size={15} /><div><strong>Needs review</strong><p>{output.review.join(' · ')}</p></div></div>}<p className="border-t border-border pt-5 font-mono text-[10px] uppercase tracking-[0.13em] text-muted-foreground">Prepared by Minuteform · Review before sharing</p></div> : <div className="flex min-h-[570px] flex-col items-center justify-center px-8 text-center"><div className="empty-icon"><FileText size={24} /></div><h3 className="mt-5 font-serif text-2xl font-semibold text-primary">Your minutes will appear here</h3><p className="mt-2 max-w-xs text-sm leading-6 text-muted-foreground">Add notes or a visual source to create a structured, share-ready record.</p></div>}
        </section>
      </div>
    </main>
  )
...
function DocumentSection({ label, content, items, numbered }: { label: string; content?: string; items?: string[]; numbered?: boolean }) { return <div className="document-section"><p className="eyebrow">{label}</p>{content && <p className="mt-2 text-sm leading-6 text-foreground">{content}</p>}{items?.length ? <ul className={`mt-2 space-y-2 text-sm leading-6 text-foreground ${numbered ? 'list-decimal pl-5' : 'list-disc pl-5'}`}>{items.map((item, i) => <li key={i}>{item}</li>)}</ul> : !content && <p className="mt-2 text-sm italic text-muted-foreground">No details captured</p>}</div> }
