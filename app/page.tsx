...
export default function Page() {
  ...
  const [applyMeetingFormat, setApplyMeetingFormat] = useState(false)
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-5 py-10 lg:grid-cols-[minmax(360px,0.82fr)_minmax(520px,1.18fr)] lg:px-8 lg:py-14">
        <section className="space-y-7">
          ...
          <div className="input-card">
            ...
            <div className="mt-5 flex items-center gap-3"><button onClick={format} disabled={!hasInput || isFormatting} className={`primary-button flex-1 justify-center ${isFormatting ? 'button-bounce' : ''}`}>{isFormatting ? <LoaderCircle className="animate-spin" size={16} /> : <Sparkles size={16} />} {isFormatting ? 'Reading source…' : 'Format minutes'}</button><button onClick={reset} aria-label="Clear source" className="icon-button"><Trash2 size={17} /></button></div>
            <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs text-muted-foreground"><label className="flex cursor-pointer items-center gap-2"><input type="checkbox" checked={applyMeetingFormat} onChange={(e) => setApplyMeetingFormat(e.target.checked)} className="h-3.5 w-3.5 accent-[var(--accent)]" /> Apply meeting format</label></div>
        ...
        <section className="document-shell" aria-live="polite">
          <div className="flex items-center justify-between border-b border-border px-6 py-5"><div><p className="eyebrow">Output / 02</p><h2 className="mt-1 font-serif text-2xl font-semibold text-primary">Meeting minutes</h2></div><div className="flex flex-wrap justify-end gap-2">{output && <><button onClick={() => setIsEditingOutput((value) => !value)} className={`secondary-button ${isEditingOutput ? 'bg-accent/10 text-accent' : ''}`} title={isEditingOutput ? 'Finish editing' : 'Edit minutes'}><Pencil size={15} /> <span className="hidden sm:inline">{isEditingOutput ? 'Done' : 'Edit'}</span></button><button onClick={() => { setOutput(null); setIsEditingOutput(false); setStatus('Output cleared. Your source notes are still available.') }} className="secondary-button" title="Clear output"><Trash2 size={15} /> <span className="hidden sm:inline">Clear</span></button><button onClick={copy} className="secondary-button" title="Copy minutes"><Clipboard size={15} /> <span className="hidden sm:inline">Copy</span></button><button onClick={async () => { setIsExporting(true); try { if (applyMeetingFormat) { await downloadMeetingPdf(output, pdfRef.current); setStatus('Meeting-format PDF exported.') } else { download(output); setStatus('MOTM exported.') } } finally { setIsExporting(false) } }} disabled={isExporting} className={`secondary-button ${isExporting ? 'button-bounce' : ''}`} title={applyMeetingFormat ? 'Export formatted PDF' : 'Download minutes'}>{isExporting ? <LoaderCircle className="animate-spin" size={15} /> : <FileText size={15} />} <span className="hidden sm:inline">{isExporting ? 'Exporting…' : `Export ${applyMeetingFormat ? 'PDF' : ''}`}</span></button></>}</div></div>
          {output ? <div className="space-y-7 p-6 sm:p-8"><div><h3 contentEditable={isEditingOutput} suppressContentEditableWarning onBlur={(e) => setOutput({ ...output, title: e.currentTarget.textContent || output.title })} className={`font-serif text-3xl font-semibold leading-tight text-primary ${isEditingOutput ? 'rounded px-1 outline outline-1 outline-accent/40' : ''}`}>{output.title}</h3><div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-xs text-muted-foreground"><span>{output.date}</span><span>{output.time}</span><span>{output.location}</span></div></div><DocumentSection label="Attendees" content={output.attendees} editable={isEditingOutput} onChange={(value) => typeof value === 'string' && setOutput({ ...output, attendees: value })} /><DocumentSection label="Discussion" items={output.discussions} editable={isEditingOutput} onChange={(value) => Array.isArray(value) && setOutput({ ...output, discussions: value })} /><DocumentSection label="Decisions" items={output.decisions} editable={isEditingOutput} onChange={(value) => Array.isArray(value) && setOutput({ ...output, decisions: value })} /><DocumentSection label="Action items" items={output.actions} editable={isEditingOutput} onChange={(value) => Array.isArray(value) && setOutput({ ...output, actions: value })} numbered />{output.review.length > 0 && <div className="review-note"><RefreshCw size={15} /><div><strong>Needs review</strong><p>{output.review.join(' · ')}</p></div></div>}<p className="border-t border-border pt-5 font-mono text-[10px] uppercase tracking-[0.13em] text-muted-foreground">Prepared by Minuteform · Review before sharing</p></div> : <div className="flex min-h-[570px] flex-col items-center justify-center px-8 text-center"><div className="empty-icon"><FileText size={24} /></div><h3 className="mt-5 font-serif text-2xl font-semibold text-primary">Your minutes will appear here</h3><p className="mt-2 max-w-xs text-sm leading-6 text-muted-foreground">Add notes or a visual source to create a structured, share-ready record.</p></div>}
        </section>
      </div>
      <div ref={pdfRef} className="meeting-pdf-template" aria-hidden="true">
        <div className="pdf-brand"><div><strong>Google Developer Groups<br />On Campus Xavier Ateneo</strong><small> Xavier University · Ateneo de Cagayan · Cagayan de Oro City</small></div><img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-1D7fE3i6sG8d1usWtxvTHmYn5rKnz0.png" alt="Google Developer Groups logo" /></div>
        <h1>MINUTES OF THE MEETING</h1>
        <table><tbody><tr><th>Meeting Details</th><th>Details</th></tr><tr><td>Meeting Title / Subject</td><td>{output?.title || 'Sample Title'}</td></tr><tr><td>Date</td><td>{output?.date || 'MM/DD/YYYY'} <span className="pdf-time">Time: {output?.time || '[Start time] - [End time]'}</span></td></tr><tr><td>Mode / Location</td><td>{output?.location || 'e.g., Discord, GMeet, Zoom, On-Campus Room'}</td></tr><tr><td>Attendees</td><td>{output?.attendees || 'e.g., Juan de la Cruz'}</td></tr><tr><td>Absentees</td><td>—</td></tr><tr><td>Roles</td><td>Facilitator: —<br />Note Taker: —</td></tr><tr><td>Time Allotted</td><td>1 hour</td></tr><tr><th colSpan={2}>Discussion</th></tr><tr><td colSpan={2} className="pdf-tall">{output?.discussions.map((item) => <div key={item}>• {item}</div>)}</td></tr></tbody></table>
        <table className="action-table"><tbody><tr><th colSpan={4}>Action Items <i>(add rows if necessary)</i></th></tr><tr><th>Task</th><th>Person Responsible</th><th>Due Date</th><th>Status</th></tr>{(output?.actions.length ? output.actions : ['Draft a project proposal…']).map((item, i) => <tr key={`${item}-${i}`}><td>{item}</td><td>—</td><td>—</td><td>□ Pending<br />□ In Progress<br />□ Completed<br />□ Overdue</td></tr>)}</tbody></table><table><tbody><tr><td>Meeting Adjourned At</td><td>[Time]</td></tr></tbody></table>
      </div>
    </main>
  )
}
...
async function downloadMeetingPdf(o: Output, node: HTMLDivElement | null) { if (!node) return; const canvas = await html2canvas(node, { scale: 2, useCORS: true, backgroundColor: '#ffffff' }); const pdf = new jsPDF({ unit: 'pt', format: 'a4' }); const width = pdf.internal.pageSize.getWidth(); const height = canvas.height * width / canvas.width; pdf.addImage(canvas.toDataURL('image/png'), 'PNG', 0, 0, width, height); pdf.save(`${o.title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-meeting-minutes.pdf`) }
